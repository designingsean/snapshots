...
/*
main snapshot javascript file
version 1.3.6 - last modified 7/27/2012 by Sean
*/
//dojo.require("esri.map");dojo.require("esri.tasks.query");dojo.require("esri.layers.FeatureLayer");dojo.require("esri.tasks.geometry");(function(a){a.fn.paneOpen=function(){return this.each(function(){a(this).removeClass("hide").animate({height:"400px"})})};a.fn.paneClose=function(){return this.each(function(){a(this).animate({height:0},function(){a(this).addClass("hide");a(".toolbar a").removeClass("active")})})}})(jQuery);Highcharts.setOptions({chart:{borderColor:"#ffffff",borderRadius:0},credits:{enabled:false},plotOptions:{pie:{point:{events:{legendItemClick:function(a){a.preventDefault()}}},states:{hover:{enabled:false}}}},title:{text:null},tooltip:{formatter:function(){var a=this.point.name;var b=Math.round(this.percentage)+"%";if(this.point.name===undefined){a=this.series.name;b=Math.round(this.y*100)/100}return"<strong>"+a+"</strong><br>"+b}},legend:{layout:"vertical",align:"right",verticalAlign:"middle"},exporting:{buttons:{printButton:{enabled:false},exportButton:{enabled:false}}}});var snapshot=(function(){var n={};var b=false,h=document.domain,m=null,c=null,g="";isMapClick=false;flexMap=null;var a=new Date();n.newFIPS=null;n.currentFIPS=null;n.newSnapshot=null;n.currentSnapshot=null;n.snapshotName=null;n.countyName=null;n.stateName=null;n.dataService="http://"+h+"/dataservices";n.mapService="http://"+h+"/arcgis/rest/services";n.queryTask=null;n.query=null;n.selectMapReady=false;function d(){var p;switch(n.currentSnapshot){case"ocean":p=n.dataService+"/Snapshot/OceanJobs?StateCountyFIPS="+n.currentFIPS;break;case"flood":p=n.dataService+"/Snapshot/FloodData?StateCountyFIPS="+n.currentFIPS;break;case"wetlands":p=n.dataService+"/Snapshot/WetlandsBenefits?StateCountyFIPS="+n.currentFIPS;break}return p}function l(p){return swfobject.getObjectById(p)}function e(){$("#pane1 button").removeAttr("disabled");queryListTask=new esri.tasks.QueryTask(n.mapService+"/Snapshot/Snapshot_Query/MapServer/1");c=new esri.tasks.Query();c.returnGeometry=false;c.outFields=["FLOOD","OCEAN_JOBS","WETLAND"];c.where="FIPS = "+n.currentFIPS;queryListTask.execute(c,function(q){var p=q.features[0].attributes;if(p.FLOOD===0){$('#pane1 button[name="flood"]').attr("disabled","disabled")}if(p.OCEAN_JOBS===0){$('#pane1 button[name="ocean"]').attr("disabled","disabled")}if(p.WETLAND===0){$('#pane1 button[name="wetlands"]').attr("disabled","disabled")}})}function j(){var p=n.newSnapshot||n.currentSnapshot;var q=n.newFIPS||n.currentFIPS;$.get(p+"/"+q+".html")}function f(){var q="10.0.0";var s="_/map/playerProductInstall.swf";var p={};var t={};t.quality="high";t.bgcolor="#ffffff";t.allowscriptaccess="sameDomain";t.allowfullscreen="true";var r={};r.id="USFlexMap";r.name="USFlexMap";r.align="middle";swfobject.embedSWF("_/map/Snapshots_FlexMap.swf?v="+a.getTime(),"flashContent","678","376",q,s,p,t,r)}function k(){queryListTask=new esri.tasks.QueryTask(n.mapService+"/Snapshot/Snapshot_Query/MapServer/1");c=new esri.tasks.Query();c.returnGeometry=false;c.outFields=["NAME","STATE_NAME","STATE_FIPS","CNTY_FIPS","FLOOD","OCEAN_JOBS","WETLAND"];c.where="1 = 1";queryListTask.execute(c,function(r){var p=[];var s=[];var q="<counties>";$.each(r.features,function(t,u){q+='<county name="'+u.attributes.NAME+'" stateName="'+u.attributes.STATE_NAME+'" stateFIPS="'+u.attributes.STATE_FIPS+'" countyFIPS="'+u.attributes.CNTY_FIPS+'" flood="'+u.attributes.FLOOD+'" ocean="'+u.attributes.OCEAN_JOBS+'" wetlands="'+u.attributes.WETLAND+'"></county>';if($.inArray(u.attributes.STATE_FIPS,s)===-1){p.push([u.attributes.STATE_FIPS,u.attributes.STATE_NAME]);s.push(u.attributes.STATE_FIPS)}});q+="</counties>";g=$.parseXML(q);$.each(p.sort(),function(t,u){$("#stateList").append('<option value="'+u[0]+'">'+u[1]+"</option>")});if((!n.newFIPS||!n.newSnapshot)&&(n.currentFIPS||n.currentSnapshot)){n.stateChange(n.currentFIPS.substr(0,2));if(n.selectMapReady){n.changeState(n.currentFIPS.substr(0,2))}}})}function o(){$(".notice").remove();if(n.currentSnapshot==="ocean"){if(n.currentFIPS==="34003"||n.currentFIPS==="34015"||n.currentFIPS==="53067"){$("body").prepend('<div class="notice">The data for this county is currently under development.</div>');$("body").css("background-position","left 42px")}}}n.init=function(){f();k()};n.mapReady=function(){n.selectMapReady=true;var p=n.newSnapshot||n.currentSnapshot;var q=n.newFIPS||n.currentFIPS;if(p!==null){n.sendSelectedSnapshot(p)}if(q!==null){n.changeState(q.substr(0,2));setTimeout(n.changeCounty(q.substr(2)),1000)}else{if($("#selectState").val()!=="0"){n.changeState($("#selectState").val())}}};n.sendSelectedSnapshot=function(p){l("USFlexMap").selectedSnapshot(p)};n.changeState=function(p){l("USFlexMap").selectedState(p)};n.changeCounty=function(p){l("USFlexMap").selectedCounty(p)};n.returnStateFIPS=function(p){snapshot.stateChange(p)};n.returnFIPS=function(p){if(p.substr(0,2)!==$("#stateList").val()){snapshot.stateChange(p.substr(0,2))}$("#countyList").val(p.substr(2));$(".close").focus();snapshot.getFIPS(p)};n.stateChange=function(p){$("#stateList").val(p);n.createCountyList(p)};n.createCountyList=function(q){$("#countyList").empty();$("#countyList").append('<option value="0">Select County</option>');var p=n.newSnapshot||n.currentSnapshot;$('county[stateFIPS="'+q+'"]['+p+'="1"]',g).each(function(){$("#countyList").append('<option value="'+$(this).attr("countyFIPS")+'">'+$(this).attr("name")+"</option>")});if((!n.newFIPS||!n.newSnapshot)&&(n.currentFIPS||n.currentSnapshot)){$("#countyList").val(n.currentFIPS.substr(2));if(n.selectMapReady){n.changeCounty(n.currentFIPS.substr(2))}}};n.getFIPS=function(p){n.newFIPS=p;if(n.currentSnapshot){$.history.load(n.currentSnapshot+"&"+n.newFIPS);$(".close").click()}else{if(n.newSnapshot){$.history.load(n.newSnapshot+"&"+n.newFIPS);$(".close").click()}else{$('.toolbar a[href="#pane1"]').click()}}};n.getSnapshot=function(p){n.newSnapshot=p;if(n.currentFIPS){$.history.load(n.newSnapshot+"&"+n.currentFIPS);$(".close").click()}else{if(n.newFIPS){$.history.load(n.newSnapshot+"&"+n.newFIPS);$(".close").click()}else{$('.toolbar a[href="#pane2"]').click()}}if($("#stateList").val()!=="0"){n.createCountyList($("#stateList").val())}if(n.selectMapReady){n.sendSelectedSnapshot(n.newSnapshot)}};n.loadContent=function(p){if(p!==""){n.screenProcess("hide");p=p.split("&");$(".download").show();$(".reset").show();n.currentFIPS=p[1];if(n.currentSnapshot!=p[0]){$("#snapshot").load(p[0]+".html",function(){b=false;n.currentSnapshot=p[0];n.startLoad()})}else{n.startLoad()}e()}};n.startLoad=function(){if(!b){n[n.currentSnapshot].createMap();b=true}n.execute();n.getData();o();j()};n.getData=function(){$.ajax({type:"GET",url:d(),dataType:"xml",success:n[n.currentSnapshot].xmlParser})};n.execute=function(){n.query.where="FIPS = "+n.currentFIPS;n.queryTask.execute(n.query,n[n.currentSnapshot].showResults)};n.findRange=function(u){var q=[];var s=[];var p,r,v,x;var w="100000000";for(i=0;i<u.length;i++){p=Math.min.apply(Math,u[i]);r=Math.max.apply(Math,u[i]);q.push(p);s.push(r)}p=Math.min.apply(Math,q);r=Math.max.apply(Math,s);v=(Math.abs(p)+Math.abs(r))/6;if(v<1){v=1}x=Math.round(v)+"";x=parseInt(w.substr(0,x.length));var t=Math.round(v/x)*x;return t};n.printPDF=function(){var p=encodeURIComponent(document.location.href);var q=encodeURIComponent(n.snapshotName+" Snapshot - "+n.countyName+", "+n.stateName);window.open("_/pdf/generatePDF.php?url="+p+"&title="+q)};n.setNames=function(r,p,q){n.snapshotName=r;n.countyName=p;n.stateName=q};n.openModal=function(r){var p=r.chart.renderTo;var q=r.chart.events;var t=$("#"+p).parent().find("p:first").text();var s="";$("#"+p).parents(".section").find("p.mapLegend").each(function(){s+=$(this).html()+"<br>"});s=s.replace(/[\(\)NA0-9\%\.]/g,"");r.chart.events={click:null};r.chart.renderTo="modal";r.exporting.enabled=true;r.legend.enabled=true;r.title.text=t;if(r.plotOptions.column){r.plotOptions.column.enableMouseTracking=true}if(r.plotOptions.line){r.plotOptions.line.enableMouseTracking=true}$.modal('<a class="modalSaveImg" onClick="modalChart.exportChart();" title="Save Chart as PNG"></a><div id="modal"></div>',{onClose:function(){r.chart.renderTo=p;r.exporting.enabled=false;r.chart.events=q;r.legend.enabled=false;r.title.text=null;if(r.plotOptions.column){r.plotOptions.column.enableMouseTracking=false}if(r.plotOptions.line){r.plotOptions.line.enableMouseTracking=false}$.modal.close()}});modalChart=new Highcharts.Chart(r)};n.screenProcess=function(p){if(p==="hide"){$("#snapshot").animate({opacity:0.25},"slow");$("body").append('<div id="process"></div>');$("#process").css("left",(($(window).width()-48)/2)+$(window).scrollLeft()+"px").css("top",(($(window).height()-48)/2)+$(window).scrollTop()+"px").fadeIn()}else{$("#snapshot").animate({opacity:1},"slow");$("#process").fadeOut(function(){$(this).remove()})}};return n}());snapshot.flood=(function(){var l={};var q,w,h,m,d,s,y,r,e,t,c,v,x,f,g,j,u,A,b,z,a;function o(){m=[],d=[],s=[],y=[],r=[],e=[],t=new Object}function n(){var B={chart:{plotBackgroundColor:null,plotBorderWidth:null,plotShadow:false},plotOptions:{pie:{enableMouseTracking:false,dataLabels:{color:"#fff",distance:-27,formatter:function(){var C=""+this.y;C=C.replace(/(\d)(?=(\d\d\d)+(?!\d))/g,"$1,");return C+"<br>"+Math.round(this.percentage)+"%"}},shadow:false,showInLegend:true,size:"100%"}},title:{text:null},legend:{enabled:false},exporting:{enabled:false}};return B}function p(){var B={chart:{renderTo:"infra",defaultSeriesType:"column"},plotOptions:{column:{enableMouseTracking:false,borderWidth:0,shadow:false,stacking:"normal"}},colors:["#6699CC","#003366"],xAxis:{categories:["Schools","Police Stations","Fire Stations","Emergency Centers","Medical Facilites","Communication Towers"],labels:{staggerLines:2,style:{fontSize:"10px",lineHeight:"10px",width:"90px"}},lineWidth:0},yAxis:{alternateGridColor:"#EAF2F5",gridLineWidth:0,title:{text:null,margin:0},tickInterval:snapshot.findRange([y[0],y[1]])},series:[{name:"Inside FEMA Floodplain",data:y[1]},{name:"Outside FEMA Floodplain",data:y[0]}],title:{text:null},legend:{enabled:false},exporting:{enabled:false}};return B}function k(){for(key in t){$("span."+key).each(function(){$(this).empty().html(t[key])})}snapshot.screenProcess("show");v=n();v.chart.renderTo="demo1";v.series=[{type:"pie",data:m}];v.colors=["#003366","#6699CC"];v.chart.events={click:function(B){snapshot.openModal(v)}};u=new Highcharts.Chart(v);x=n();x.chart.renderTo="demo2";x.series=[{type:"pie",data:d}];x.colors=["#003366","#6699CC"];x.chart.events={click:function(B){snapshot.openModal(x)}};A=new Highcharts.Chart(x);f=n();f.chart.renderTo="demo3";f.series=[{type:"pie",data:s}];f.colors=["#003366","#6699CC"];f.chart.events={click:function(B){snapshot.openModal(f)}};b=new Highcharts.Chart(f);infraOptions=p();infraOptions.chart.events={click:function(B){snapshot.openModal(infraOptions)}};infraChart=new Highcharts.Chart(infraOptions);c=parseFloat(t.Acres_Converted);if(c===-1){$("#env1").html('<p class="nochange">No Data Available</p>').css("background-image","url(_/img/nochange.png)").css("cursor","default");$("#env2").html('<p class="nochange">No Data Available</p>').css("background-image","url(_/img/nochange.png)").css("cursor","default");$(".envChart .legendBlock").parent().css("visibility","hidden")}else{if(c===0){$("#env1").html('<p class="nochange">No Change Detected</p>').css("background-image","url(_/img/nochange.png)").css("cursor","default");$("#env2").html('<p class="nochange">No Change Detected</p>').css("background-image","url(_/img/nochange.png)").css("cursor","default");$(".envChart .legendBlock").parent().css("visibility","hidden")}else{$("#env1").empty().css("background-image","none").css("cursor","pointer");$("#env2").empty().css("background-image","none").css("cursor","pointer");$(".envChart .legendBlock").parent().css("visibility","visible");g=n();g.chart.renderTo="env1";g.series=[{type:"pie",data:r}];g.colors=["#003366","#6699CC"];g.chart.events={click:function(B){snapshot.openModal(g)}};z=new Highcharts.Chart(g);j=n();j.chart.renderTo="env2";j.series=[{type:"pie",data:e}];j.colors=["#9EC54E","#4C662E"];j.chart.events={click:function(B){snapshot.openModal(j)}};a=new Highcharts.Chart(j)}}}l.createMap=function(){snapshot.queryTask=new esri.tasks.QueryTask(snapshot.mapService+"/Snapshot/Flood_BacksideMap/MapServer/7");if(q){dijit.byId("floodmap_infowindow").destroy()}q=new esri.Map("floodmap",{slider:false,logo:false});dojo.connect(q,"onLoad",function(){q.disableMapNavigation()});snapshot.query=new esri.tasks.Query();snapshot.query.returnGeometry=true;snapshot.query.outFields=["FIPS"]};l.showResults=function(D){var E=esri.graphicsExtent(D.features);E=E.expand(1.4);q.setExtent(E);var C=D.features[0].attributes.FIPS;var B=[];B[6]="STCOFIPS = "+C;B[7]="FIPS = "+C;h=new esri.layers.ImageParameters();h.layerDefinitions=B;w=new esri.layers.ArcGISDynamicMapServiceLayer(snapshot.mapService+"/Snapshot/Flood_BacksideMap/MapServer",{imageParameters:h});q.addLayer(w)};l.xmlParser=function(B){o();$(B).find("FloodData").each(function(){var C=$(this)[0].attributes;for(i=0;i<C.length;i++){t[C[i].nodeName]=C[i].nodeValue}});snapshot.setNames("Flood Exposure",t.County_Name,t.State_Name);m.push(["Outside FEMA Floodplain",parseFloat(t.Out_FP_Pop)]);m.push(["Inside FEMA Floodplain",parseFloat(t.In_FP_Pop)]);d.push(["Outside FEMA Floodplain",parseFloat(t.Out_FP_Over65)]);d.push(["Inside FEMA Floodplain",parseFloat(t.In_FP_Over65)]);s.push(["Outside FEMA Floodplain",parseFloat(t.Out_FP_Poverty)]);s.push(["Inside FEMA Floodplain",parseFloat(t.In_FP_Poverty)]);y.push([parseFloat(t.SCHOOL_OUT),parseFloat(t.POLICE_OUT),parseFloat(t.FIRE_OUT),parseFloat(t.EOC_OUT),parseFloat(t.MEDICAL_OUT),parseFloat(t.COMMUNICATION_OUT)]);y.push([parseFloat(t.SCHOOL_IN),parseFloat(t.POLICE_IN),parseFloat(t.FIRE_IN),parseFloat(t.EOC_IN),parseFloat(t.MEDICAL_IN),parseFloat(t.COMMUNICATION_IN)]);r.push(["Outside FEMA Floodplain",parseFloat(t.Out_FP_Acres_Converted)]);r.push(["Inside FEMA Floodplain",parseFloat(t.In_FP_Acres_Converted)]);e.push(["Agricultural Area",parseFloat(t.Acres_Ag2Dev)]);e.push(["Natural Area",parseFloat(t.Acres_Nat2Dev)]);k()};return l}());snapshot.ocean=(function(){var h={};var F,n,A,H,y,r,w,j,K,I,z,g,G,b,f,e,s,u,x,J,d,E,M,t,c,m,C,l,D=[["Living Resources","Marine Construction","Marine Transportation","Offshore Mineral Extraction","Ship and Boat Building","Tourism and Recreation","Suppressed"],["#67B8E0","#FF9E5F","#B94B3E","#FDE260","#CDEA64","#6969B4","#cccccc"],["living","construction","transport","mineral","ship","tourism","suppress"]],a=true,B=true;function v(){A=[],H=[],y=[],r=[],w=0,j=0,K=[],I=[],z=[],g=[],G=[],b=new Object,f=new Object,e=new Object;a=true;B=true}function o(T,O){var R=[];var Q=0;var N=0;while((Q===0)||(Q===null)){Q=O[N];N++}N--;for(var S=0;S<A.length;S++){var P;if(N>S||O[S]===null){P=null}else{if(T=="percent"){P=((O[S]/O[N])-1)*100}else{P=O[S]-O[0]}}if(isNaN(P)){P=null}R.push(P)}return R}function q(){var N={chart:{plotBackgroundColor:null,plotBorderWidth:null,plotShadow:false},plotOptions:{pie:{enableMouseTracking:false,dataLabels:{distance:13,formatter:function(){if(this.y>0){return this.y+"%"}else{return null}}},size:"70%",borderWidth:0,shadow:false,showInLegend:true}},colors:D[1],title:{text:null},legend:{enabled:false},exporting:{enabled:false}};return N}function L(){var N={chart:{defaultSeriesType:"line",borderColor:"#ffffff"},plotOptions:{line:{enableMouseTracking:false,marker:{symbol:"circle"},shadow:false}},colors:D[1],xAxis:{labels:{style:{fontSize:"10px"}},lineWidth:0},yAxis:{alternateGridColor:"#EAF2F5",gridLineWidth:0,title:{text:null,margin:0}},title:{text:null},legend:{enabled:false},exporting:{enabled:false}};return N}function p(){var N={chart:{renderTo:"wages1",defaultSeriesType:"column",marginBottom:70,spacingBottom:15},plotOptions:{column:{enableMouseTracking:false,borderWidth:0,groupPadding:0.1,pointPadding:0,shadow:false}},colors:["#003366","#6699CC"],xAxis:{categories:g[2],labels:{staggerLines:3,style:{fontSize:"10px"}},lineWidth:0},yAxis:{alternateGridColor:"#EAF2F5",gridLineWidth:0,title:{text:null,margin:0}},series:[{name:"County",data:g[0]},{name:"Nation",data:g[1]}],title:{text:null},legend:{enabled:false},exporting:{enabled:false}};return N}function k(){for(key in e){$("span."+key).each(function(){$(this).empty().html(e[key])})}for(key in b){var N=$.inArray(key,D[0]);$("span."+D[2][N]+"Sector").each(function(){$(this).html(b[key])});$("span."+D[2][N]+"Trends").each(function(){$(this).html(f[key])})}snapshot.screenProcess("show");s=q();s.chart.renderTo="sector1";s.series=[{type:"pie",data:H}];s.chart.events={click:function(O){snapshot.openModal(s)}};M=new Highcharts.Chart(s);u=q();u.chart.renderTo="sector2";u.series=[{type:"pie",data:y}];u.chart.events={click:function(O){snapshot.openModal(u)}};t=new Highcharts.Chart(u);x=q();x.chart.renderTo="sector3";x.series=[{type:"pie",data:r}];x.chart.events={click:function(O){snapshot.openModal(x)}};c=new Highcharts.Chart(x);J=L();J.chart.renderTo="trends1";J.xAxis.categories=A;if(!B){J.yAxis.tickInterval=snapshot.findRange([I[0].data,I[1].data,I[2].data,I[3].data,I[4].data,I[5].data])}else{J.colors=[D[1][6]]}J.series=I;J.chart.events={click:function(O){snapshot.openModal(J)}};m=new Highcharts.Chart(J);d=L();d.chart.renderTo="trends2";d.xAxis.categories=A;if(!B){d.yAxis.tickInterval=snapshot.findRange([z[0].data,z[1].data,z[2].data,z[3].data,z[4].data,z[5].data])}else{d.colors=[D[1][6]]}d.series=z;d.chart.events={click:function(O){snapshot.openModal(d)}};C=new Highcharts.Chart(d);E=p();E.chart.events={click:function(O){snapshot.openModal(E)}};l=new Highcharts.Chart(E)}h.createMap=function(){snapshot.queryTask=new esri.tasks.QueryTask(snapshot.mapService+"/Snapshot/OceanJobs_BacksideMap/MapServer/2");if(F){dijit.byId("oceanmap_infowindow").destroy()}F=new esri.Map("oceanmap",{slider:false,logo:false});dojo.connect(F,"onLoad",function(){F.disableMapNavigation()});n=new esri.layers.ArcGISDynamicMapServiceLayer(snapshot.mapService+"/Snapshot/OceanJobs_BacksideMap/MapServer");F.addLayer(n);snapshot.query=new esri.tasks.Query();snapshot.query.returnGeometry=true};h.showResults=function(N){var O=esri.graphicsExtent(N.features);O=O.expand(2.4);F.setExtent(O)};h.xmlParser=function(P){v();$(P).find("CountyData").each(function(){var T=$(this)[0].attributes;for(i=0;i<T.length;i++){e[T[i].nodeName]=T[i].nodeValue}var S=parseInt(e.MinAnalysisYear);var R=parseInt(e.AnalysisYear);for(i=S;i<=R;i++){A.push(i)}});snapshot.setNames("Ocean Jobs",e.County_Name,e.State_Name);$(P).find("JobsPercentages SectorJobs").each(function(R){H.push([D[0][R],parseFloat($(this).attr("PercentEmployment"))]);w+=parseFloat($(this).attr("PercentEmployment"));b[$(this).attr("OceanSector")]=$(this).attr("PercentEmployment_Display")});if(w<100){H.push([D[0][6],Math.round((100-w)*100)/100]);b.suppress=Math.round((100-w)*100)/100+"%";e.suppressSector=Math.round((100-w)*100)/100+"%"}else{e.suppressSector="NA"}$(P).find("JobsPercentagesForState SectorJobs").each(function(R){y.push([D[0][R],parseFloat($(this).attr("PercentEmployment"))]);j+=parseFloat($(this).attr("PercentEmployment"))});if(j<100){y.push([D[0][6],Math.round((100-j)*100)/100])}$(P).find("JobsPercentagesForNation SectorJobs").each(function(R){r.push([D[0][R],parseFloat($(this).attr("PercentEmployment"))])});for(i=0;i<D[0].length;i++){var N=[];$(P).find('JobsDecade Jobs[OceanSector="'+D[0][i]+'"]').each(function(){var R=$(this).attr("Employment");if(R==="-9999"){R=null}else{R=parseFloat(R)}if((R!==0)&&(R!==null)){B=false}N.push(R)});I.push({name:D[0][i],data:o("percent",N)});z.push({name:D[0][i],data:o("absolute",N)})}if(B){I=[];z=[];I.push({name:D[0][6],data:[0,0,0,0,0]});z.push({name:D[0][6],data:[0,0,0,0,0]})}$(P).find("SectorJobsPercentIncrease_Display SectorJobs").each(function(R){f[$(this).attr("OceanSector")]=$(this).attr("JobsPercentIncrease")});var Q=[];var O=[];$(P).find("SectorAnnualWages SectorWages").each(function(R){var S=parseFloat($(this).attr("AnnualWages"));if(isNaN(S)){Q.push(null)}else{Q.push(S)}G.push($(this).attr("OceanSector"))});$(P).find("SectorAnnualWagesForNation SectorWages").each(function(R){var S=parseFloat($(this).attr("AnnualWages"));if(isNaN(S)){O.push(null)}else{O.push(S)}});g.push(Q);g.push(O);g.push(G);k()};return h}());snapshot.wetlands=(function(){var j={};var k,b,f,l,g,d;var h=new esri.tasks.GeometryService("http://www.csc.noaa.gov/arcgis/rest/services/Geometry/GeometryServer");j.mapExtent=[];function a(){l=[],displayVals=new Object}function e(){var m={chart:{plotBackgroundColor:null,backgroundColor:"rgba(0,0,0,0)",plotBorderWidth:null,plotShadow:false},plotOptions:{pie:{enableMouseTracking:false,dataLabels:{distance:13,formatter:function(){return this.y+"%"}},size:"70%",shadow:false,showInLegend:true,slicedOffset:5}},title:{text:null},legend:{enabled:false},exporting:{enabled:false}};return m}function c(){for(key in displayVals){$("span."+key).each(function(){$(this).empty().html(displayVals[key])})}if(displayVals.GDP_County==="unavailable*"){$(".productiveChart p.mapLegend").show()}else{$(".productiveChart p.mapLegend").hide()}snapshot.screenProcess("show");g=e();g.chart.renderTo="safer2";g.series=[{type:"pie",data:l}];g.chart.events={click:function(m){snapshot.openModal(g)}};d=new Highcharts.Chart(g)}j.createMap=function(){snapshot.queryTask=new esri.tasks.QueryTask(snapshot.mapService+"/Snapshot/Wetlands_Snapshot_Data/MapServer/1");if(k){dijit.byId("wetlandsmap_infowindow").destroy()}k=new esri.Map("wetlandsmap",{slider:false,logo:false});dojo.connect(k,"onLoad",function(){k.disableMapNavigation()});snapshot.query=new esri.tasks.Query();snapshot.query.returnGeometry=true;snapshot.query.outFields=["FIPS"]};j.showResults=function(p){var q=esri.graphicsExtent(p.features);q=q.expand(1.1);k.setExtent(q);var o=p.features[0].attributes.FIPS;var n=[];n[2]="FIPSSTCO != "+o;n[1]="FIPS != "+o;f=new esri.layers.ImageParameters();f.layerDefinitions=n;b=new esri.layers.ArcGISDynamicMapServiceLayer(snapshot.mapService+"/Snapshot/Wetlands_Snapshot_Data/MapServer",{imageParameters:f});k.addLayer(b);var m=new esri.SpatialReference({wkid:4326});h.project([q],m,function(r){j.mapExtent=r[0]})};j.xmlParser=function(m){a();$(m).find("CountyData").each(function(){var n=$(this)[0].attributes;for(i=0;i<n.length;i++){displayVals[n[i].nodeName]=n[i].nodeValue}});$(m).find("CountyData").children().children().each(function(){var n=$(this)[0].attributes;for(i=0;i<n.length;i++){displayVals[n[i].nodeName]=n[i].nodeValue}});snapshot.setNames("Wetland Benefits",displayVals.County_Name,displayVals.State_Name);if(displayVals.WetlandsPercentTotal!=="0"){l.push({name:"Wetlands",y:parseFloat(displayVals.WetlandsPercentTotal),color:"#6FA359",sliced:true,selected:true})}if(displayVals.DevelopedPercentTotal!=="0"){l.push({name:"Developed",y:parseFloat(displayVals.DevelopedPercentTotal),color:"#9732CC",sliced:false,selected:false})}if(displayVals.AgriculturePercentTotal!=="0"){l.push({name:"Agriculture",y:parseFloat(displayVals.AgriculturePercentTotal),color:"#EFAE50",sliced:false,selected:false})}if(displayVals.OtherPercentTotal!=="0"){l.push({name:"Other (grasslands, forests,<br>scrub vegetation, and<br>barren land)",y:parseFloat(displayVals.OtherPercentTotal),color:"#dddddd",sliced:false,selected:false})}c()};return j}());dojo.ready(function(){$(document).ready(function(){snapshot.init();$.history.init(snapshot.loadContent);$(".toolbar a").click(function(){var a=$(this).attr("href");if((snapshot.newSnapshot!==null||snapshot.currentSnapshot!==null)||a!=="#pane2"){$(".panes div[id]").addClass("hide");$(a).removeClass("hide");$(".toolbar a").removeClass("active");$(this).addClass("active");if($(".panes").height()===0){$(".panes").paneOpen();$(".close").slideDown()}}else{$.modal("<p><strong>Select a Snapshot First</strong></p><p>In order to ensure that we have data avaiable for your county, please select a Snapshot first.</p>",{containerId:"selectFirst",position:["48px"]})}return false});$(".close").click(function(){$(this).slideUp(function(){$("#welcome").slideUp();$(this).css("float","none")});$(".panes").paneClose()});$("#pane1 button").click(function(){snapshot.getSnapshot($(this).attr("name"));return false});$("#stateList").change(function(){snapshot.createCountyList($(this).val());if(snapshot.selectMapReady){snapshot.changeState($(this).val())}});$("#countyList").change(function(){var a=$("#stateList").val()+$(this).val();if(snapshot.selectMapReady){snapshot.changeCounty($(this).val())}snapshot.getFIPS(a)});$(".download").click(function(){snapshot.printPDF()});$(".reset").click(function(a){window.open("/snapshots/","_self")});$(".largerMap a").live("click",function(){window.open($(this).attr("href")+"&extent="+snapshot.wetlands.mapExtent.xmin+","+snapshot.wetlands.mapExtent.ymin+","+snapshot.wetlands.mapExtent.xmax+","+snapshot.wetlands.mapExtent.ymax);return false});$("#floodRef").live("click",function(a){a.preventDefault();snapshot.getSnapshot("flood");return false});$("#oceanRef").live("click",function(a){a.preventDefault();if(snapshot.currentFIPS==="22005"||snapshot.currentFIPS==="22007"||snapshot.currentFIPS==="22033"||snapshot.currentFIPS==="22053"||snapshot.currentFIPS==="24027"||snapshot.currentFIPS==="37103"||snapshot.currentFIPS==="37147"){alert("The Ocean Jobs Snapshot is not available for this county at this time.")}else{snapshot.getSnapshot("ocean")}return false});if(snapshot.currentFIPS===null){$('.toolbar a[href="#pane1"]').click()}else{$(".close").slideDown()}})});
